...
class ReplaceTables extends ProcessPluginBase {
  ...
  */
  public function transform($value, MigrateExecutableInterface $migrate_executable, Row $row, $destination_property) {
    ...
    $matches = [];
    preg_match_all('/(\[NJS_TABLE:.*?\])/', $value, $matches, PREG_SET_ORDER);
    /*
    ...
    if (!empty($matches)) {
      foreach ($matches as $match) {
        list($placeholder, $harvest_mig_uuid, $table_uuid) = explode(':', str_replace(array('[', ']'),'' , $match[1]));
        ...
        $uri = ConfigUtil::GetHarvesterBaseUrl() . 'get-harvest-item/' .  $mig_uuid . '/component_table/' . $table_uuid;
        $headers = [
        ...
        if (isset($response->rows->table_markup)) {
          $markup = $response->rows->table_markup;
